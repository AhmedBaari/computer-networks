...
class TCPClient extends JFrame implements ActionListener, MouseListener {
			...
			try {
				...
				if (s.equals("Success")) {
					...
					while ((c = inFromServer.read()) != -1) {
						...
						// CHANGE: since they used FILEOUT instead of DATAOUT
						fileOut.write(c);
						fileOut.flush();
	...
	public static void main(String args[]) {
		// if at least three argument are passed, consider the first one as directory
		// path,
		...
		if (args.length >= 3) {
			...
			System.out.println(
					"Please enter the client directory address as first argument while running from command line.");
		}
	}
}
